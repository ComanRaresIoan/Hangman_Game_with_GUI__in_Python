...
class Hangman:
    def __init__(self): # We initialize the attributes here
        self.quit = False # quit attribute
        self._played_games = 0 # played games attribute
        self._won_games = 0 # won games attribute
        self._wrong_guesses = 0 # wrong guesses attribute
        self._target_word = self._select_word() # target word attribute
        self._guessed_letters = set() # guessed letters attribute
        self._canvas = None # canvas attribute
        self._guessed_word = "" # guessed word
    ...
    def _build_letters_frame(self):
        ...
        letter_buttons = [[
            sg.Button(
                ...
                key=f"-letter-{letter}-",
                enable_events=True,
            ) for letter in letter_group] for letter_group in letter_groups]
        return sg.Column(
            [[sg.Frame("Letters", letter_buttons, font="Any 20"), sg.Sizer()]]
        )

    def _build_guessed_word_frame(self):
        return sg.Frame(
            "",
            [[sg.Text(key="-DISPLAY-WORD-", font="Courier 20")]],
